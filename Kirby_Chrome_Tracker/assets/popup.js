function loadWebsitesDatabase(){
  chrome.storage.sync.get('websites',function(obj){
    ...
    if(websites.length == 0){
      ...
      website_added_count.html(websites.length+'website is added<br><br>author: BadBoy17G');
    ...
    if(current_page_indexed == false){
      $('#add-current-site').html('<a class="reg-button">Track '+this_page_domain+'</a>');
    }
	});
}
function addCurrentSiteToWatchlist(){
  chrome.storage.sync.get('websites',function(obj){
    ...
    if(websites.length >= 120){
      ...
      response_msg = "Limit Reached";
    }

    if(index_new_site == true){
      websites.push({"d":this_page_domain,"t":0});
      response_msg = "Added "+this_page_domain;
    }

    var sync_workaround = {"websites":websites};

    chrome.storage.sync.set(sync_workaround,function(){
      // Set done
    });

    $("#add-current-site").html('<div class="add-response">'+response_msg+'</div>');
  });
}
...
function stopTrackingSite(domain_to_del){
  chrome.storage.sync.get('websites',function(obj){
    ...
    $('.website-list .site[data-website-domain="'+domain_to_del+'"]').slideUp(400);
	});
}
...
$(document).on('click','#add-current-site .reg-button',function(){
  addCurrentSiteToWatchlist();
});
$(document).on('click','.website-list .site .delete-icon',function(){
  var domain = $(this).closest('.site').data('website-domain');
